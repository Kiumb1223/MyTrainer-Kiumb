...
class Tracker:
    ...
    def update_node(self,node_feat):
        self.node_feats_list.append(node_feat)
    ...
    def age(self) -> int:
        ...
        return self.frame_idx - self.start_frame
...
class TrackManager:
    ...
    def update(self,cur_frame:int,current_detections:np.ndarray,img_date:torch.Tensor) -> List[Tracker]:
        ...
        det_graph  = self.construct_det_graph(current_detections[first_dets_list],img_date)
        ...
        if match_idx and len(match_idx[0]) > 0 :         # matched tras and dets 
            ...
            det_conf_list  = [current_detections[first_dets_list[i]][4] for i in det_idx]
        ...
        for tra_id in unmatch_tra:   # unmatched tras 
            ...
            first_tras_list[tra_id].bt_prev_node = False
        ...
        if second_tras_list :
            if second_dets_list:
                ...
                #---------------------------------#
                tmp_det_graph = self.construct_det_graph(current_detections[second_dets_list],img_date)
                self.model.gen_appFeats(tmp_det_graph)
                # second_dets_node = tmp_det_graph.node_feats[second_dets_list]
                tmp_det_graph.geometric_info = tmp_det_graph.geometric_info.cpu().numpy()
                second_dets_conf = current_detections[second_dets_list,4]
                match_mtx,match_idx,unmatch_tra,unmatch_det = self._iou_reid_match(
                        second_tras_list,tmp_det_graph.geometric_info[:,:4],
                        tmp_det_graph.x,self._second_match_thresh
                    )
                ...
                if match_idx and len(match_idx[0]) > 0 :    # matched tras and dets
                    ...
                    det_app_feats  = tmp_det_graph.x[det_idx]
                    ...
                    for i,(tra_id, det_id) in enumerate(zip(tra_idx,det_idx)):
                        second_tras_list[tra_id].to_active(
                            cur_frame,smooth_app_feats[i].squeeze(),
                            det_conf_list[i],tmp_det_graph.geometric_info[det_id]
                        )
        ...
        if match_idx and len(match_idx[0]) > 0 :         # matched tras and dets 
            ...
            det_conf_list  = [current_detections[third_dets_list[i]][4] for i in det_idx]
        ...
        for det_id in unmatch_det:
            if current_detections[third_dets_list[det_id]][4] >= self._det2tra_conf:
                third_tras_list.append(
                    Tracker(
                        cur_frame,third_dets_app[det_id],
                        third_dets_node[det_id],current_detections[third_dets_list[det_id]][4],
        ...
        #                   Update NODE feats
        #------------------------------------------------------------------#
        if output_track_list:
            tmp_tra_graph  = self.construct_tra_graph(output_track_list)
            tra_node_feats = self.model.gen_nodeFeats(tmp_tra_graph)
            for idx,track in enumerate(output_track_list):
                new_node_feats = tra_node_feats[idx]

                if track.age > self._max_cnt_to_dead / 2:
                    # when the track is too old, the node feats will be directly updated
                    track.update_node(new_node_feats)
                else:
                    smooth_feats = self.smooth_feature(track.node_feats_list[-1],new_node_feats,None,None,'EMAFF')
                    track.update_node(smooth_feats)
    ...
    def construct_det_graph(self,dets_list :np.ndarray ,img_date:torch.Tensor) -> Data:
    ...
    def smooth_feature(self,
        ...
        if smooth_feature.dim() == 2:
            return smooth_feature.split(1,0)
        elif smooth_feature.dim() == 1:
