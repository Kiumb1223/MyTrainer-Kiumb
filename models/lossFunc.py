...
class GraphLoss(nn.Module):
    ...
    def dice_loss(self,pred:Tensor,target:Tensor):
        ...
        intersection = (pred_labels * target).sum()
        dice = (2.0 * intersection + self.eps) / (pred_labels.sum() + target.sum() + self.eps)
        return 1 - dice
    ...
    def forward(self, pred_mtx_list: list, gt_mtx_list: list) -> Tensor:
        ...
        for i in range(num_graphs):
            ...
            gt_mtx = gt_mtx_list[i].float()
            pred_mtx = pred_mtx_list[i].float() 
...
class CombinedMatchingLoss(nn.Module):
    ...
    def forward(self, 
            pred_mtx_list: list, gt_mtx_list: list,
            pred_dsmat: Tensor = None, gt_perm: Tensor = None,
            src_ns: Tensor = None, tgt_ns: Tensor = None
        ) -> Tensor:
        """
        组合损失计算
        """
