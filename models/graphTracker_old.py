...
class TrackManager:
    ...
    #             first_match_list[tra_id].to_active(cur_frame,smooth_features[i].squeeze(),
    ...
    #     match_mtx,match_idx,unmatch_tra,unmatch_det = self._iou_match(second_match_list,highconf_unmatch_dets.copy(),self._third_match_thresh)
    #     if match_idx and len(match_idx[0]) > 0 :         # matched tras and dets 
    #         tra_feats,tra_conf_list = [] , []
    #         det_feats , det_geometric_info = [], []
    #         tra_idx ,det_idx = match_idx
    #         for i in tra_idx:
    #             tra_feats.append(second_match_list[i].appearance_feats_list[-1])
    #             tra_conf_list.append(second_match_list[i].conf)
    #         tra_feats = torch.stack(tra_feats).to(self.device).to(torch.float32)

    #         for det_id in det_idx:
    #             global_id = highconf_to_global_det_idx[det_id]
    #             det_feats.append(det_graph.x[global_id])
    #             det_geometric_info.append(det_graph.geometric_info[global_id].cpu().numpy())
    #         det_feats = torch.stack(det_feats,dim=0)
    #         det_conf_list = [highconf_unmatch_dets[i][4] for i in det_idx]
    #         smooth_features = self.smooth_feature(tra_feats,det_feats,tra_conf_list,det_conf_list)          

    #         for i,( tra_id, det_id ) in enumerate(zip(tra_idx,det_idx)):
    #             second_match_list[tra_id].to_active(cur_frame,smooth_features[i].squeeze(),det_conf_list[i],det_geometric_info[i])
    ...
    #             Tracker(cur_frame,det_graph.x[global_id],
    #                     highconf_unmatch_dets[det_id][4],det_graph.geometric_info[global_id].cpu().numpy(),
    ...
    #         patch = T.crop(im_tensor,int(y),int(x),int(h),int(w))
    ...
    # def _iou_match(self,tracks_list,highconf_unmatch_dets:np.ndarray,match_thresh:float):      
    ...
    #     if self.fusion_method == 'DFF' or not prev_feats.numel(): # Direct Feature Fusion OR EMPTY TRAGRAPH
